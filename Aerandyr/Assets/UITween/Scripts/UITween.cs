...
namespace UITween
{
    ...
    public class PositionPropetiesAnim
    {
        ...
        public void SetPosStart(Vector3 StartPos, RectTransform rectTr)
        {
			...
			Transform rootObject = rectTr.transform;
        ...
        public void SetPosEnd(Vector3 EndPos, Transform rectTr)
        {
            this.EndPos = EndPos;
#if UNITY_EDITOR
			//var root = rectTr.root;
			var root = rectTr.transform;
			EndWorldPos.x = StartWorldPos.x + (EndPos.x - StartPos.x) * root.localScale.x;
            EndWorldPos.y = StartWorldPos.y + (EndPos.y - StartPos.y) * root.localScale.y;
#endif
        }

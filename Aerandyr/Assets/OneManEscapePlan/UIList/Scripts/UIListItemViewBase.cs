...
namespace OneManEscapePlan.UIList.Scripts {
	...
	[System.Serializable]
	abstract public class UIListItemViewBase<T> : MonoBehaviour {

		[System.Serializable] public class UIListItemViewEvent : UnityEvent<UIListItemViewBase<T>> { }
		...
		public T Model {
			...
			set {
				...
				if (model != null && model is IBindable) {
					(model as IBindable).ChangeEvent.AddListener(Refresh);
				}
		...
		protected void removeBinding() {
			if (model != null && model is IBindable) {
				(model as IBindable).ChangeEvent.RemoveListener(Refresh);
			}
		}
	}
}
